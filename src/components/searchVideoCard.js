    ...
    <div className="min-w-[83vw] max-w-[98vw] overflow-y-scroll scrollbar-hidden max-h-[85vh] pt-5">
      <div className="grid grid-cols-1 gap-5 p-5 min-h-screen">
        {filtered?.items?.length > 0 ? (
          ...
          // )
          <Virtuoso
            totalCount={filtered?.items?.length}
            className="scrollbar-hidden min-h-[100vh] lg:min-h-[79vh]"
            itemContent={(index) => {
              const item = filtered?.items[index];
              return (
                <div
                  className="shadow-md lg:shadow-none lg:hover:shadow-md gap-5 p-5 rounded-lg w-full max-h-full"
                  key={index}
                >
                  <VideoCard1 info={item} />
                </div>
          ...
          <div className="min-h-screen dark:text-[#f1f1f1] dark:bg-[#0f0f0f]">
            <p className="dark:text-[#f1f1f1] text-4xl">No videos found.</p>
          </div>
        )}
      </div>
    </div>
